...
public abstract class LoginActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        Delete = (Button) findViewById(com.uoc.openapilibrary.R.id.btnDelete);
    ...
    protected static String refreshToken() {
        ...
        try {
            request = OAuthClientRequest
                    ...
                    .setClientId(settings.getString("oauthClient","")).setClientSecret(settings.getString("oauthSecret",""))
                    .setRedirectURI(Constants.REDIRECT_URI).setRefreshToken(settings.getString("oauthRefresh",""))
                    .buildBodyMessage();
        } catch (OAuthSystemException e) {
            e.printStackTrace();
            return null;
        ...
        try {
            ...
            if (response.getAccessToken() != null) {
                ...
                settings.edit().putString("oauthRefresh", response.getRefreshToken()).commit();
                settings.edit().putString("oauthToken", response.getAccessToken()).commit();
            }
        ...
        protected void onPostExecute(String result) {
            if (result == null) {
                ...
                try {
                    ...
                    settings.edit().putString("oauthName", json.getString("name")).commit();
                    settings.edit().putString("oauthClient", json.getString("client")).commit();
                    settings.edit().putString("oauthSecret", json.getString("secret")).commit();
